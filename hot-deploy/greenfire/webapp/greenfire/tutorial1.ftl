...
<main >
  <div id="main">
	  <ul>
		...
		<li>
			<p>This line tells us that treedoor is a type of Door, where Door is the class with methods open and close. So we can program treedoor using the methods open and close in the Door class. Note that treedoor is a single word and there is no space between the words tree and door. In the Python programming language, it is also common to create new words using an underscore, so the word treedoor could also be written as tree_door. The case of the letters in each word is important. The name treedoor is written using lower case letters only - changing the case of any letter in this word will result in an error in the program and it will not run.  The class name Door starts with an uppercase letter, so you should follow this convention in the program. </p>		
		</li>
		<li>
			<p>This brings us to the last line of the program, which will cause the treedoor to close when the program is run:</p>
		</li>
		<li>
			<p>
				<pre>
