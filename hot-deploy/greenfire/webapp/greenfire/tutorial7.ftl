...
<main id="main">
  <section >
      To solve the next problem, you will need to learn how for loops work. In addition, you will also learn about a programming structure called list that is used to store data. Python has two types of loops: while and for. You've already learned how to use the while loop. Likewise, the for loop is also used to perform repetitious tasks. The next example shows a simple program that has a for loop to print out the numbers 0 to 9:</br>
...
<code>root_veggies = ['cassava', 'earthnut', 'lily yam', 'rutabaga']</code>
